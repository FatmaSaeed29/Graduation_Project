                ...
                <form action="" method="GET">
                  <div class="input-group mb-3 container" style="margin-top:200px;" dir="rtl">
                    <input type="text" name="search"  value="<?php if(isset($_GET['search'])){echo $_GET['search']; } ?>"
                     class="form-control rounded-right" placeholder="إبحث هنا بالسجل التجاري او الضريبي " style="border-top-left-radius:0px;border-bottom-left-radius:0px; border-top-right-radius:20px; border-bottom-right-radius:20px;">
        ...
        <table class="table container table-bordered border-danger animals"  style="margin-top:50px;">
            ...
            <?php
                ...
                if(isset($_GET['search']))
                {
                  ...
                  $queryone = "SELECT * FROM company WHERE CONCAT(tax,commercial_num) LIKE '%$filtervalues%' ";
                  $query_run = mysqli_query($conn, $queryone);
                      ...
                      foreach($query_run as $items)
                        {
                            ...
                            <tr>
                              <td><?= $i ?></td>
                              <td><?= $items['company_activity']; ?></td>
                              <td><?= $items['tax']; ?></td>
                              <td><?= $items['commercial_num']; ?></td>
                              <td><?= $items['employees_num']; ?></td>
                              <td><?= $items['request_num']; ?></td>
                              <td><?= $items['request_date']; ?></td>
                              <td><?= $items['categories']; ?></td>
                              <td><?= $items['email']; ?></td>
                              <td><?= $items['government']; ?></td>
                              <td><?= $items['city']; ?></td>
                              <td><?= $items['center']; ?></td>
                              <td><?= $items['address']; ?></td>
                            </tr>
